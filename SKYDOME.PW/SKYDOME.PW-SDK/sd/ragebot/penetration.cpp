...
namespace F::AUTOWALL {
    ...
    void F::AUTOWALL::c_auto_wall::pen(data_t& data, const Vector local_pos, const Vector target_pos, C_BaseEntity* target, C_BaseEntity* local, C_CSPlayerPawn* localpawn, C_CSPlayerPawn* targetpawn,
    ...
    void F::AUTOWALL::c_auto_wall::ScaleDamage(data_t& data, const int hitgroup, C_CSPlayerPawn* player)
    {
        ...
        static CConVar* mp_damage_scale_ct_head = g_interfaces->EngineCVar->Find("mp_damage_scale_ct_head"),
            * mp_damage_scale_t_head = g_interfaces->EngineCVar->Find( "mp_damage_scale_t_head"),
            * mp_damage_scale_ct_body = g_interfaces->EngineCVar->Find("mp_damage_scale_ct_body"),
            * mp_damage_scale_t_body = g_interfaces->EngineCVar->Find( "mp_damage_scale_t_body");
    ...
    {
     //   L_PRINT(LOG_INFO) << "0";
        SD_ASSERT(data.m_local != nullptr || data.m_target != nullptr || data.m_wpn_data != nullptr || data.m_local_pawn != nullptr || data.m_target_pawn != nullptr);
    ...
    //    L_PRINT(LOG_INFO) << "1";
        ...
        g_interfaces->Trace->Init(filter, data.m_local_pawn,PENMASK,  3, 7);
        ...
        */
        g_interfaces->Trace->CreateTrace(&trace_data, data.m_pos.at(data_t::e_pos::e_local), end_pos, filter, 4);
      ...
      //  L_PRINT(LOG_INFO) << "Initialized handlebulletpen data";
        ...
        if (trace_data.m_num_update > 0) {
            for (int i{ }; i < trace_data.m_num_update; i++) {
                ...
                g_interfaces->Trace->InitializeTraceInfo(&game_trace);
                g_interfaces->Trace->get_trace_info(
                ...
                */
                ...
                if (g_interfaces->Trace->handle_bullet_penetration(&trace_data, &handle_bullet_data, value, false))                
                    return false;
