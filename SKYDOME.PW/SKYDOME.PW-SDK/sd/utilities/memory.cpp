...
bool MEM::Setup()
{
	...
	fnSetRelativeMouseMode = reinterpret_cast<decltype(fnSetRelativeMouseMode)>(GetExportAddress(hSDL3, XorStr("SDL_SetRelativeMouseMode")));
	bSuccess &= (fnSetRelativeMouseMode != nullptr);

	fnGetRelativeMouseMode = reinterpret_cast<decltype(fnGetRelativeMouseMode)>(GetExportAddress(hSDL3, XorStr("SDL_GetRelativeMouseMode")));
	bSuccess &= (fnGetRelativeMouseMode != nullptr);

	fnSetWindowMouseGrab = reinterpret_cast<decltype(fnSetWindowMouseGrab)>(GetExportAddress(hSDL3, XorStr("SDL_SetWindowMouseGrab")));
	bSuccess &= (fnSetWindowMouseGrab != nullptr);

	fnWarpMouseInWindow = reinterpret_cast<decltype(fnWarpMouseInWindow)>(GetExportAddress(hSDL3, XorStr("SDL_WarpMouseInWindow")));
	bSuccess &= (fnWarpMouseInWindow != nullptr);

	fnLoadKV3 = reinterpret_cast<decltype(fnLoadKV3)>(GetExportAddress(hTier0, XorStr("?LoadKV3@@YA_NPEAVKeyValues3@@PEAVCUtlString@@PEBDAEBUKV3ID_t@@2@Z")));
	bSuccess &= (fnLoadKV3 != nullptr);

	fnCreateMaterial = reinterpret_cast<decltype(fnCreateMaterial)>(FindPattern(MATERIAL_SYSTEM2_DLL, XorStr("48 89 5C 24 ? 48 89 6C 24 ? 56 57 41 56 48 81 EC ? ? ? ? 48 8B 05")));
	bSuccess &= (fnCreateMaterial != nullptr);

	fnUtlBufferInit = reinterpret_cast<decltype(fnUtlBufferInit)>(GetExportAddress(hTier0, XorStr("??0CUtlBuffer@@QEAA@HHH@Z")));
	bSuccess &= (fnUtlBufferInit != nullptr);

	fnUtlBufferPutString = reinterpret_cast<decltype(fnUtlBufferPutString)>(GetExportAddress(hTier0, XorStr("?PutString@CUtlBuffer@@QEAAXPEBD@Z")));
	bSuccess &= (fnUtlBufferPutString != nullptr);

	fnUtlBufferEnsureCapacity = reinterpret_cast<decltype(fnUtlBufferEnsureCapacity)>(GetExportAddress(hTier0, XorStr("?EnsureCapacity@CUtlBuffer@@QEAAXH@Z")));
...
void* MEM::GetModuleBaseHandle(const wchar_t* wszModuleName)
{
	...
	if (pModuleBase == nullptr)
		LOG(ERROR) << XorStr("module base not found: \"") << wszModuleName << XorStr("\"");
...
void* MEM::GetExportAddress(const void* hModuleBase, const char* szProcedureName)
{
	...
	{
		LOG(ERROR) << XorStr("module has no exports: \"") << GetModuleBaseFileName(hModuleBase) << XorStr("\"");
		return nullptr;
	}
...
#ifdef _DEBUG
			LOG(INFO) << XorStr("export found: \"") << reinterpret_cast<const char*>(pBaseAddress + pNamesRVA[uMiddle]) << XorStr("\" in \"") << GetModuleBaseFileName(hModuleBase) << XorStr("\" at: ") << std::hex /*L::AddFlags(LOG_MODE_INT_SHOWBASE | LOG_MODE_INT_FORMAT_HEX)  */ << XorStr("0x") << uFunctionRVA;
#else
			LOG(INFO) << XorStr("export found: ") << szProcedureName;
#endif // _DEBUG
	...
	LOG(ERROR) << XorStr("export not found: ") << szProcedureName;
...
bool MEM::GetSectionInfo(const void* hModuleBase, const char* szSectionName, std::uint8_t** ppSectionStart, std::size_t* pnSectionSize)
{
	...
	LOG(ERROR) << XorStr("code section not found: \"") << szSectionName << XorStr("\"");
	return false;
}
...
std::uint8_t* MEM::FindPattern(const wchar_t* wszModuleName, const char* szBytePattern, const char* szByteMask)
{
	...
	if (hModuleBase == nullptr)
	{
		LOG(ERROR) << XorStr("failed to get module handle for: \"") << wszModuleName << XorStr("\"");
		return nullptr;
	}
	...
	if (pIDH->e_magic != IMAGE_DOS_SIGNATURE)
	{
		LOG(ERROR) << XorStr("failed to get module size, image is invalid");
		return nullptr;
	}
	...
	if (!vecFoundOccurrences.empty())
	{
		...
		if (vecFoundOccurrences.size() > 1U)
		{
			...
			L_PRINT(LOG_WARNING) << XorStr("found more than one occurrence with \"") << szPattern << XorStr("\" pattern, consider updating it!");
	...
	if (pFoundAddress == nullptr)
	{
		...
		LOG(ERROR) << XorStr("pattern not found: \"") << szPattern << XorStr("\"");
