...
/*virtual ~trace_filter_t() {}
	...
	trace_filter_t(C_CSPlayerPawn* skip, uint64_t mask, uint8_t layer, uint16_t idk)
	{
		...
		static function_t fn = reinterpret_cast<function_t>(MEM::FindPattern(CLIENT_DLL, XorStr("48 89 5C 24 08 48 89 74 24 10 57 48 83 EC 20 0F B6 41 37 33")));
		// way better sig  
		if (fn == nullptr) {
			L_PRINT(LOG_WARNING) << XorStr("trace_filter_t Invalid signature!");
		}

		SD_ASSERT(fn != nullptr);
...
public:
	...
	CS2_PAD(1, 0x24)
	Vector m_start_pos, m_end_pos, m_normal, m_pos;
	MEM_PAD(0x4);
	float Fraction;
	MEM_PAD(0x6);
	bool m_all_solid;
...
public:	// cHoca
	...
	void Init(trace_filter_t& filter, /*C_CSPlayerPawn*/void* skip, uint64_t mask, uint8_t layer, uint16_t idk)
	{
		//initfilter_19A770((__int64)filter, a2, 536577i64, 4, 7);
		using function_t = trace_filter_t*(__fastcall*)(trace_filter_t&, void*, uint64_t, uint8_t, uint16_t);
		static function_t fn = reinterpret_cast<function_t>(MEM::FindPattern(CLIENT_DLL, XorStr("48 89 5C 24 08 48 89 74 24 10 57 48 83 EC 20 0F B6 41 37 33")));
	...
	//abaixo da funcao
	void ClipTraceToPlayers(Vector& start, Vector& end, trace_filter_t* filter, game_trace_t* trace, float min, int length, float max)
	{	// cHoca

		using function_t = void(__fastcall*)(Vector&, Vector&, trace_filter_t*, game_trace_t*, float, int, float);
		static function_t fn = reinterpret_cast<function_t>(MEM::FindPattern(CLIENT_DLL, CLIP_TRACE_TO_PLAYERS));
		// way better sig  
		if (fn == nullptr) {
			//L_PRINT(LOG_WARNING) << XorStr("ClipTraceToPlayers creating new signature!");
	...
	// client.dll; 48 89 5C 24 08 48 89 6C 24 10 48 89 74 24 18 57 48 83 EC 60 48 8B E9 0F
	 ...
	 static void get_trace_info(trace_data_t* trace, game_trace_t* hit,
		...
		static function_t fn = reinterpret_cast<function_t>(MEM::FindPattern(CLIENT_DLL, XorStr("48 89 5C 24 08 48 89 6C 24 10 48 89 74 24 18 57 48 83 EC 60 48 8B E9 0F")));
	...
	static bool handle_bullet_penetration(trace_data_t* const trace, void* stats,
		...
		static function_t fn = reinterpret_cast<function_t>(MEM::FindPattern(CLIENT_DLL, XorStr("48 8B C4 44 89 48 20 55 57 41 55")));

		return fn(trace, stats, mod_value, nullptr, nullptr, nullptr, nullptr, nullptr, draw_showimpacts);
	}

	static void CreateTrace(trace_data_t* const trace, const Vector start,
		const Vector end, const trace_filter_t& filler,
		const int penetration_count) {


		using function_t = void(__fastcall*)(trace_data_t*, Vector, Vector,
			trace_filter_t,  int);
		static function_t fn = reinterpret_cast<function_t>(MEM::FindPattern(CLIENT_DLL, XorStr("48 89 5C 24 08 48 89 6C 24 10 48 89 74 24 18 57 41 56 41 57 48 83 EC 40 F2")));
		SD_ASSERT(fn != nullptr);
		return fn(trace, start, end, filler, penetration_count);
	}

	// #STR: "Physics/TraceShape (Client)" then xref
	void TraceShape(ray_t& ray, Vector* start, Vector* end, trace_filter_t filter, game_trace_t& trace)
	{
		using function_t = bool(__fastcall*)(void*, ray_t&, Vector*, Vector*, trace_filter_t, game_trace_t&);
